...
impl<'a> RosbagPlayer<'a>{
    pub fn new(path: &str, ros: &'a mut Ros) -> Self {
        let t_path = path;
        ...
        for record in records {
            ...
            match iterated {
                Record::Connection(conn) => {
                    let topic_publisher = self.ros.publish(conn.topic).unwrap();
                    ...
                    info!("rosbag_player {}: id {} with topic {} of type {} inserted to publishers map",
            ...
            match iterated {
                Record::Chunk(chunk) => {
                    ...
                    for msg in chunk.iter_msgs() {
                        ...
                        info!("rosbag_player {}: publish msg {:?} with decoded data {:?} of type {} into topic {}",
