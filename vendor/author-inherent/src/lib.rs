...
decl_module! {
    pub struct Module<T: Config> for enum Call where origin: T::Origin {
        ...
        fn set_author(origin, author: H160) {
            ensure_none(origin)?;
        ...
        fn on_finalize(_n: T::BlockNumber) {
            assert!(Author::get().is_some(), "No valid author set in block");
        }
    }
}
...
impl<T: Config> ProvideInherent for Module<T> {
    ...
    fn create_inherent(data: &InherentData) -> Option<Self::Call> {
        ...
        let author =
            H160::decode(&mut &author_raw[..]).expect("Decodes author raw inherent data");
