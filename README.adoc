...
image:https://github.com/airalab/robonomics/workflows/Integration/badge.svg?branch=master["CI Status", link="https://github.com/airalab/robonomics/actions"]
...
Robonomics project implements a software platform and includes a set of open-source packages and infrastructure for Robotics, Smart Cities and Industry 4.0 developers.
...
robonomics
 ...
 . Generate session keys during RPC call at https://parachain.robonomics.network/#/toolbox[Toolbox page]:
...
robonomics --validator
 ...
 . Generate accounts at https://parachain.robonomics.network/#/accounts/create[Account page]:
 .. create `stash` usign `Schnorrkel (sr25519)` crypto;
 .. create `controller` using `Schnorrkel (sr25519)` crypto.

 . Request testnet XRT in our https://matrix.to/#/#robonomics:matrix.org[Matrix room].
 . Transfer testnet XRT to `stash` and a little bit to `controller` account.

 . https://parachain.robonomics.network/#/staking/actions[Staking actions]:
 ...
 . Check https://parachain.robonomics.network/#/staking[Validator List], at new Era validator set will updated.
