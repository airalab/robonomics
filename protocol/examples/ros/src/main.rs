...
async fn main() {
    ...
    let behaviour = Ros::new(RosConfig::new());
    ...
    if let Some(addr) = std::env::args().nth(1) {
        let remote: Multiaddr = addr.parse().expect("correct multiaddress");
        swarm.dial(remote).expect("dialable node");
        println!("Dialed {}", addr);
    }
...
mod ros {
    ...
    // pub const PROTOCOL_NAME: &[u8] = b"/robonomics-ros/ping/1.0.0";
