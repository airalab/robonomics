...
use crate::network::{
    NetworkEndpoints, ASSET_HUB_PARA_ID, PARA_ACCOUNT, PARA_ID, PARA_SIB_ACCOUNT,
};
...
pub async fn test_hrmp_create_channel(endpoints: &NetworkEndpoints) -> Result<()> {
...
async fn test_create_foreign_asset(endpoints: &NetworkEndpoints) -> Result<()> {
...
pub async fn test_xcm_token_teleport(topology: &NetworkTopology) -> Result<()> {
    ...
    match topology {
        NetworkTopology::AssetHub => {
            ...
            log::info!("[ 1/4 ] Test: open HRMP channel with AssetHub");
            test_hrmp_create_channel(&endpoints).await?;

            log::info!("[ 2/4 ] Test: create foreign asset on AssetHub");
            test_create_foreign_asset(&endpoints).await?;
