...
async fn test_simple_tree(ws_url: &str) -> Result<()> {
    ...
    let child_node = Node::create(&client, Some(root_node.id()), Some(child_meta), Some(child_payload))
        .await
...
async fn test_complex_tree(ws_url: &str) -> Result<()> {
    ...
    if root_info.children.len() == NODE_COUNT {
        log::info!("✓ Complex tree structure verified ({} children)", NODE_COUNT);
    } else {
        anyhow::bail!("Expected {} children, found {}", NODE_COUNT, root_info.children.len());
    }
...
async fn test_encrypted_payloads(ws_url: &str) -> Result<()> {
    ...
    for i in 0..5 {
        ...
        // Encrypt the data
        let encrypted_msg = sender_cipher.encrypt(
            plaintext.as_bytes(),
            &receiver_public,
