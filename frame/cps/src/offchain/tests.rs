...
fn test_node_operation_encoding() {
    ...
    assert_eq!(
        decoded.operation,
        OperationType::Move(Some(NodeId(1)), NodeId(2))
    );
...
fn test_operation_type_enum() {
    ...
    assert_eq!(
        OperationType::decode(&mut &create.encode()[..]).unwrap(),
        create
    );
    assert_eq!(
        OperationType::decode(&mut &create_with_parent.encode()[..]).unwrap(),
        create_with_parent
    );
    assert_eq!(
        OperationType::decode(&mut &move_op.encode()[..]).unwrap(),
        move_op
    );
    assert_eq!(
        OperationType::decode(&mut &delete.encode()[..]).unwrap(),
        delete
    );
}
